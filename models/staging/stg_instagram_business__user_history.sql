...
is_most_recent as (
    ...
    select 
        *,
        row_number() over (partition by user_id {{ fivetran_utils.partition_by_source_relation(package_name='instagram_business') }} order by _fivetran_synced desc) = 1 as is_most_recent_record
    from final
